...
public class CaipinOrderView extends CaipinOrderEntity implements Serializable {
    private static final long serialVersionUID = 1L;
		...
		* 取餐类型的值
		*/
		private String caipinOrderStatusValue;
		/**
		* 订单类型的值
		*/
	...
	public CaipinOrderView(CaipinOrderEntity caipinOrderEntity) {
		try {
			...
			* 获取： 取餐类型的值
			*/
			public String getCaipinOrderStatusValue() {
				return caipinOrderStatusValue;
			}
			/**
			* 设置： 取餐类型的值
			*/
			public void setCaipinOrderStatusValue(String caipinOrderStatusValue) {
				this.caipinOrderStatusValue = caipinOrderStatusValue;
			}
			/**
			* 获取： 订单类型的值
			*/
