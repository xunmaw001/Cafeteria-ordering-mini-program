        ...
        <el-form
            ...
            <input id="yonghuId" name="yonghuId" type="hidden">
               <el-col :span="12">
                ...
                <el-col :span="12">
                    <el-form-item class="select" v-if="type!='info'"  label="取餐类型" prop="caipinOrderStatusTypes">
                        <el-select v-model="ruleForm.caipinOrderStatusTypes" :disabled="ro.caipinOrderStatusTypes" placeholder="请选择取餐类型">
                            <el-option
                                v-for="(item,index) in caipinOrderStatusTypesOptions"
                    ...
                    <div v-else-if="type=='info'">
                        <el-form-item class="input" label="取餐类型" prop="caipinOrderStatusValue">
                        <el-input v-model="ruleForm.caipinOrderStatusValue"
                            placeholder="取餐类型" readonly></el-input>
                        </el-form-item>
                    </div>
                </el-col>
               <el-col :span="12">
...
<script>
    ...
    export default {
        data() {
            return {
                ...
                rules: {
                   ...
                   caipinOrderStatusTypes: [
                              { required: true, message: '取餐类型不能为空', trigger: 'blur' },
                              {  pattern: /^[0-9]*$/,
                                  message: '只允许输入整数',
                                  trigger: 'blur'
                              }
        ...
        created() {
            ...
            //获取下拉框信息
                this.$http({
                    url:`dictionary/page?page=1&limit=100&sort=&order=&dicCode=caipin_order_status_types`,
                    method: "get"
                }).then(({ data }) => {
                    if (data && data.code === 0) {
                        this.caipinOrderStatusTypesOptions = data.data.list;
                    }
                });
                this.$http({
