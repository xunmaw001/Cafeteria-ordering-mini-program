	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback">
		...
		<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			<view class="search-form round">
				<input v-model="searchForm.gonglueName" type="text" placeholder="攻略名称"></input>
			</view>
		...
		<scroll-view scroll-x="true" class="tabView"
			...
			<view :style='categoryName===item.codeIndex?sactiveItem:sitem' class="tab"
				v-for="(item,index) in gonglueTypesList" :key="index"
		...
		<view class="list" :style='{"borderRadius":0,"backgroundColor":"rgba(255, 255, 255, 0)"}'>
			<view style="margin: 10rpx 10rpx 10rpx 10rpx;" @tap="onDetailTap(product)" class="listm flex flex-between"
				:style='{"borderRadius":"40rpx","backgroundColor":"#A27DE3"}' v-for="(product,index) in list"
				:key="index">
				<image :style='{"borderRadius":"40rpx","width":"50%","height":"200rpx"}' mode="aspectFill"
					class="listmpic" :src="product.gongluePhoto?product.gongluePhoto.split(',')[0]:''"></image>
				<view class="listmr">
				...
				<view v-if="product.yonghuId == user.id" style="display: flex;justify-content: space-between;float: left;margin-top: 20%;">
					<text style="margin-left: 50rpx;" v-if="isAuth('gonglue','修改')" class="cuIcon-edit"
						@click.stop="onUpdateTap(product.id)">
						修改
					</text>
					<text v-if="isAuth('gonglue','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除
					</text>
				</view>
			</view>
		</view> <button
			...
			v-if="isAuth('gonglue','新增')" class="add-btn" @click="onAddTap()">新增</button>
...
<script>
	export default {
		...
		async onShow() {
			...
			//当前表的 攻略类型 字段 字典表查询方法
			let gonglueTypes = await this.$api.page('dictionary', {
				page: 1,
				limit: 100,
				dicCode: 'gonglue_types'
			});
			this.gonglueTypesList = gonglueTypes.data.list;
		...
		methods: {
			...
			async upCallback(mescroll) {
				...
				if (this.categoryName != 0) {
					params.gonglueTypes = this.categoryName
				}

				let res = await this.$api.list(`gonglue`, params);
			...
			onDetailTap(item) {
				this.$utils.jump(`../gonglue/detail?id=${item.id}`)
			},
			// 修改
			onUpdateTap(id) {
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('gonglue', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search() {
				...
				if (this.searchForm.gonglueName) {
					searchForm['gonglueName'] = this.searchForm.gonglueName
				}
				if (this.searchForm.gonglueName) {
					searchForm['gonglueName'] = this.searchForm.gonglueName
				}

				let res = await this.$api.list(`gonglue`, searchForm);
				// 如果是第一页数据置空
