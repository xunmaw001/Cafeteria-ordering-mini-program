<template>
	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input v-model="searchForm.gonglueLiuyanText" type="text" placeholder="评论内容"></input>
		</view>
	...
	<view class="list b-b" v-for="(item, index) in list" :key="index">
		<view class="wrapper" style="margin-left: 25upx;margin-top: 10upx;width: 800rpx;">
			<view style="font-size: 30rpx;">
				<b>{{item.gonglueName}}</b>
			</view>
			<view class="address-box" style="margin-bottom: 20rpx;width: 600rpx;">
				<text style="margin-left: 30rpx;">我的评论：{{item.gonglueLiuyanText}}</text>
			</view>
		</view>
        <view style="float: right;margin-right: 25upx;">
            <text v-if="isAuth('gonglueLiuyan','删除')" style="margin: 0 50upx 0 30rpx;" class="cuIcon-delete" @click.stop="onDeleteTap(item.id)"></text>
        </view>
	...
	<button
            ...
            v-if="isAuth('gonglueLiuyan','新增')" class="add-btn" @click="onAddTap()">新增</button>
...
<script>
	export default {
		...
		methods: {
			//查询条件切换
			queryChange(e) {
				this.searchForm.gonglue_liuyanName = "";
			},
			//类别搜索
			...
			async upCallback(mescroll) {
                ...
                let res = await this.$api.page(`gonglueLiuyan`, params);
			...
			onDetailTap(item) {
				this.$utils.jump(`../gonglueLiuyan/detail?id=${item.gonglueLiuyanId}`)
			},
			// 修改
			onUpdateTap(id) {
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('gonglueLiuyan', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search() {
				...
				if (this.searchForm.gonglueLiuyanText) {
                    searchForm['gonglueLiuyanText'] = this.searchForm.gonglueLiuyanText
                }

                let res = await this.$api.page(`gonglueLiuyan`, searchForm);
				// 如果是第一页数据置空
