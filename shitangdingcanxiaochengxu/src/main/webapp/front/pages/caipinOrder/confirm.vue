<template>
    <view class="content">
        <form>
            <view @tap="onAddressTap" v-if="caipinOrderStatusTypes === '2'" class="cu-form-group">
			...
			<view class="cu-form-group">
			    ...
			    <view class="uni-list">
					<radio-group @change="radioChange">
						<label v-for="(item, index) in items" :key="item.codeIndex">
								<radio :value="item.codeIndex+''" :checked="item.codeIndex === caipinOrderStatusTypes" />
							{{item.indexName}}
						</label>
					</radio-group>
				</view>
...
<script>
    export default {
        ...
        async onLoad(options) {
			...
			this.items = caipinTypes.data.list;
        ...
        methods: {
			 radioChange: function(evt) {
				this.caipinOrderStatusTypes = evt.detail.value
			},
            async onSubmitTap() {
                let _this = this;
                let table = uni.getStorageSync("nowTable");
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            if(_this.addresszhi == null && _this.caipinOrderStatusTypes === '2'){
							...
							if(_this.caipinOrderStatusTypes === '2'){
								 data.addressId= _this.addresszhi.id
								 data.caipins= JSON.stringify(_this.orderGoods)
								 data.yonghuId= _this.user.id
								 data.caipinOrderStatusTypes = _this.caipinOrderStatusTypes
								 data.caipinOrderPaymentTypes=  _this.caipinOrderPaymentTypes
							}else{
								 data.caipins= JSON.stringify(_this.orderGoods)
								 data.yonghuId= _this.user.id
								 data.caipinOrderStatusTypes = _this.caipinOrderStatusTypes
